...
class ItemTable {
        ...
        if ($this->filter) {
            ...
            if (!empty($condition)) {
                foreach ($condition as $key => $value) {
                    $condition[$key] = $this->db->real_escape_string($value);
                }
    ...
    public function editClassroom($id, $photo, $number, $campus, $furniture, $students) {
        ...
        $stmt = $this->db->prepare($sql);
        $stmt->bind_param('ssisii', $this->db->real_escape_string($photo), $this->db->real_escape_string($number), intval($campus), $this->db->real_escape_string($furniture), intval($students), intval($id));
        $stmt->execute();
    }
